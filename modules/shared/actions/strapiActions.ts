import axios from 'axios';

import { ServerAddress, Strapi } from "../../../enum/url";

export const getStrapiPages = async (page: string , acceptLanguage: "fa-IR"|"en-US"|"ar-AE" = "fa-IR") => {

    const token = "";
    try {
        const response = await axios({
            method: "get",
            url: `${ServerAddress.Type}${ServerAddress.Strapi}${Strapi.Pages}?filters[Page][$eq]=${page}&populate[0]=Sections`,
            headers: {
                "Accept-Language": acceptLanguage,
                Authorization: `bearer ${token}`
            }
        });
        return (response)
    } catch (error: any) {
        return error
    }
}

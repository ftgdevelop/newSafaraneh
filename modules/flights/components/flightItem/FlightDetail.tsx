...
const FlightDetailItem: React.FC<any> = ({ FlightsData }: { FlightsData: FlightItemType }) => {
    ...
    const today =  dateDisplayFormat({date:dateFormat(new Date()), locale:'fa',format:'ddd dd mm'})
    ...
    return (
        <div className="grid grid-cols-3 max-sm:grid-cols-2 justify-items-center pt-3 pb-2 max-sm:pl-2 max-sm:pr-2 gap-3 bg-gray-100">
            <div>
                ...
                <p className="text-2xs max-sm:text-4xs text-gray-500">{dateDisplayFormat({date:(query.departing as string), locale:'fa',format:'ddd dd mm'})|| today}</p>
            </div>
            <div>
